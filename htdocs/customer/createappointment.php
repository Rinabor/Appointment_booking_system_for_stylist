...
// Check if the customer already has a pending or approved appointment
$has_active_appointment = false;

$sql = "SELECT COUNT(*) FROM Appointment WHERE CustomerID = ? AND Status IN ('Pending', 'Accepted')";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["specialization"])) {
    ...
    $sql = "SELECT StylistID, Name FROM Stylist WHERE Specialization = ? ORDER BY Name";
    if ($stmt = $conn->prepare($sql)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["stylist"], $_POST["appointment_date"], $_POST["start_time"])) {
    if ($has_active_appointment) {
        $message = "<p style='color: red; font-weight: bold;'>You already have an active appointment (Pending or Accepted). Complete or cancel your existing appointment before booking a new one.</p>";
    } else {
...
<body>
    ...
    <div class="container">
        ...
        <?php if ($has_active_appointment): ?>
            <p style="color: red; font-weight: bold;">You already have an active appointment (Pending or Accepted). Complete or cancel your existing appointment before booking a new one.</p>
        <?php else: ?>
            ...
            <?php if (!empty($stylists)) { ?>
                <form action="createappointment.php" method="POST">
                    ...
                    <input type="date" id="appointment_date" name="appointment_date" value="<?php echo isset($preferences['date']) ? $preferences['date'] : ''; ?>" required>
                    
                    <label for="start_time">Start Time:</label>
                    <input type="time" id="start_time" name="start_time" value="<?php echo isset($preferences['time']) ? $preferences['time'] : ''; ?>" required>
                    ...
                    <div class="stylist-table">
                        <?php foreach ($stylists as $stylist) { ?>
                            <div class="stylist-card <?php echo (isset($preferences['stylist_id']) && $preferences['stylist_id'] == $stylist['id']) ? 'selected' : ''; ?>" 
                                ...
                                <img src="../stylist/uploads/<?php echo $stylist['id']; ?>.jpg" 
                                     alt="Stylist Image" 
                                     onerror="this.src='../stylist/uploads/default.jpg';">
